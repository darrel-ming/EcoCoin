...
void MainWindow::BackUptoUSB()
{
    ...
    foreach (const QStorageInfo &storage, QStorageInfo::mountedVolumes())
    {
        ...
        {
            //qDebug() << "yep" << "/n";
            m_strBackupUsbPath = storage.rootPath().contains(usbstring);
        ...
        if (m_strBackupUsbPath.toLatin1() == "")
        {
            //date
            QFile::copy("/settings.txt", m_strBackupUsbPath.toLatin1());
            QFile::copy("/coins.sqlite", m_strBackupUsbPath.toLatin1());
            QFile::copy("/availableCoins.sqlite", m_strBackupUsbPath.toLatin1());
            QFile::copy("/hashes.txt", m_strBackupUsbPath.toLatin1());
...
int MainWindow::getkeys()
{ //for coldstorage server or standalone server which contains all the infos
    ...
    if (ui->usbdrivename->text().toLatin1() != "")
    {
        ListUSB();
        QString path2;
        path2 = m_strUsbPath.toLatin1() + "/keys.txt";
        QFile MyFile2(path2);
        if (MyFile2.exists())
        {
            keyexists = true;
    ...
    if (MyFile.exists() && keyexists)
    {
        ...
        m_strCoinKey = nums.at(1);
        qDebug() << "coinkey" << m_strCoinKey;
        test = nums.at(2);
        ...
        QString testmd5 = md5Checksum("masterkey:" + masterkey.toLatin1() + "coinkey:" + m_strCoinKey.toLatin1() + "coinsmd5:" + fileChecksum("coins.sqlite", QCryptographicHash::Md5)).toHex();
