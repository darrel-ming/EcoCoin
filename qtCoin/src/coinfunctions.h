...
void MainWindow::on_SendCoins_clicked()
{
    ...
    { // user is specified
        ...
        { //when second user not specified send coins
...
{                                                            // place into client walled based on yearly tables when created ID
    ...
    {   //receiving coins
        ...
        for (int i = 0; i < ammount.toInt(); i++)
        {
            ...
            if (test != "")
            { // coin not from rcoins needs decryption first
                signedcoins << test;
            }
            else
            {
    ...
    { ///send coins to yeardb wallet
        ...
        QString query2 = "INSERT INTO "
                         "'" +
                         euserid.toLatin1() + "'"
                                              "(origindex,addr,datetime,class,hold) VALUES (?,?,1,0,0)";
        ...
        if (missingcoin == 0)
        { // if no missing coins validated then remove from rcoins
            ...
            for (int i = 0; i < ammount.toInt(); i++)
            {
                query3.exec("SELECT * FROM coins WHERE addr LIKE "
                            "'" +
                            coins.at(i).toString().toLatin1() + "'"
                                                                "ORDER BY random()");
                while (query3.next())
                {
                    ...
                    if (query3.value(2).toString().toLatin1() == coins.at(i).toString().toLatin1())
                    {
                        ...
                        query3.exec("DELETE FROM coins WHERE id ="
                                    "'" +
...
int MainWindow::checkAllCoins(QString db2)
{
    ...
    m_DB.setDatabaseName(db2.toLatin1());
    m_DB.open();
    m_DB.transaction();
    QSqlQuery query;
...
int MainWindow::checkAvailableCoins(QString ID, QString db2, QString needed)
{
    ...
    {
        if (m_bAdmin)
        { // look in id for available coins
        }
        //check wallet
    }
...
void MainWindow::on_actionOpenCoin_triggered()
{
    ...
    {
        printf("actionOpenCoin Err:%s", e.what());
    }
}
