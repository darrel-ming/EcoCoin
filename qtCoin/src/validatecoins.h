...
QString MainWindow::generateTXfile(QString suserid, QString ruserid, QString etxcoins)
{ //file to send from client
    ...
    if (m_bAdmin)
    { // maybe check if frontend or backend
    ...
    if (ruserid.toLatin1() == "")
    { // if receiver is blank then check admin mode
        if (m_bAdmin)
        {   // send to user already done with placecoins
            //     placecoins();
        }
        else
        { // inbetween personal accounts from main wallet ID or pull from all addresses.  might not be needed use send button instead

            m_DB.setDatabaseName("./wallet.sqlite");
            m_DB.open();
            QSqlDatabase::database().transaction();
            QSqlQuery query;
    ...
    { //send from address to address check if any of wallet addresses match for multiple wallets the same for timed offline transactions
...
QString MainWindow::generateRXfile(QString ruserid, QString suserid, QString etxcoins)
{ //rxfile to give client encrypted coins to put in wallet might not be needed. for client only cold server
...
QString MainWindow::validateCOINsignWallet(QString ID, QString Coin)
{ //for offline server validation
...
QString MainWindow::validateCOINsign(QString coin, QString userID)
{ // for getting coins from rcoins and placing into userid
    ...
    if (userID.toLatin1() == "")
    {   // only check rcoins and coins for unencrypted coins
        ...
        m_DB.transaction();
        QSqlQuery query4;
        query4.exec("SELECT * FROM users WHERE name = "
                    "'" +
                    userID.toLatin1() + "'"
                                        " AND addr = " +
                    coin.toLatin1());
        ...
        QSqlDatabase::database().transaction();
        QSqlQuery query5;
        query5.exec("SELECT * FROM coins WHERE addr="
                    "'" +
                    coin.toLatin1() + "'");
        while (query5.next())
        {
    ...
    { ///check user exists and get signing info to sign coins with
        qDebug() << "signing coin then getting another";
        if (coin.length() > 8)
        { //if encrypted unsign coin
            qDebug() << "encrypted coin during verify unsingning";
            m_DB.setDatabaseName("wallet.sqlite"); //search for signed coin in db then unsign for placement and resigning
            m_DB.open();
            QSqlDatabase::database().transaction();
            QSqlQuery query2;
            query2.exec("SELECT * FROM users WHERE coin = "
                        "'" +
                        userID.toLatin1() + "'");
            while (query2.next())
            {
        ...
        QSqlDatabase::database().transaction();
        QSqlQuery query2;
        query2.exec("SELECT * FROM users WHERE userid = "
                    "'" +
                    userID.toLatin1() + "'");
        while (query2.next())
        {
        ...
        QSqlDatabase::database().transaction();
        QSqlQuery query3;
        query3.exec("SELECT addr FROM coins WHERE trim(addr) ="
                    "'" +
                    coin.toLatin1() + "'"); // match and contains // collate binary  trim(addr)
        ...
        while (query3.next())
        {
            ...
            if (query3.value(0).toString().toLatin1() == coin.toLatin1())
            {
                ...
                if (1)
                {
                    ...
                    QString ecoin = simplecrypt(signedcoin.toLatin1(), m_strCoinKey.toLatin1(), QCryptographicHash::Sha512);
...
{   // can validate public encrypted and master encrypted ID's
    ...
    while (query2.next())
    {
        ...
        if (userid.toLatin1() == query2.value(7).toString().toLatin1())
        { // check if userid matches ekey or public key
    ...
    m_strYearDb = userid.left(4);
    qDebug() << m_strYearDb;

    //verify decrypted id

    m_DB.setDatabaseName("./db/" + m_strYearDb.toLatin1() + ".sqlite");
    //   db.open();
    if (m_DB.open())
    {
